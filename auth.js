...
const authorizationCodeMissingFromServerResponse =
...
function buildClientInfoFromHomeAccountId(homeAccountId) {
  ...
  const clientInfoParts = homeAccountId.split(
  ...
  return {
    ...
    utid:
      clientInfoParts.length < 2 ? Constants.EMPTY_STRING : clientInfoParts[1],
  };
}
...
function tenantIdMatchesHomeTenant(tenantId, homeAccountId) {
  return (
    !!tenantId && !!homeAccountId && tenantId === homeAccountId.split('.')[1]
  );
}
/**
...
function getDeserializedResponse(responseString) {
  ...
  try {
    ...
    const deserializedHash = Object.fromEntries(
...
function getAliasesFromStaticSources(staticAuthorityOptions, logger) {
  ...
  if (canonicalAuthority) {
    const authorityHost = new UrlString(canonicalAuthority).getUrlComponents()
      .HostNameAndPort;
    staticAliases =
...
class CacheManager {
  ...
  getTenantedAccountInfoByFilter(
    ...
    if (tenantProfileFilter) {
      if (
        !this.tenantProfileMatchesFilter(tenantProfile, tenantProfileFilter)
      ) {
        return null;
      }
    }
  ...
  isCredentialKey(key) {
    ...
    if (
      lowerCaseKey.indexOf(CredentialType.ID_TOKEN.toLowerCase()) === -1 &&
  ...
  getAccessToken(
    ...
    const credentialType =
      authScheme &&
      authScheme.toLowerCase() !== AuthenticationScheme.BEARER.toLowerCase()
  ...
  readAppMetadataFromCache(environment) {
    ...
    const appMetadataEntries = Object.keys(appMetadata).map(
      (key) => appMetadata[key]
    );
  ...
  matchUserAssertionHash(entity, userAssertionHash) {
    return !!(
      entity.userAssertionHash && userAssertionHash === entity.userAssertionHash
    );
  }
  /**
...
class RequestValidator {
  ...
  static validateCodeChallengeMethod(codeChallengeMethod) {
    if (
      [CodeChallengeMethodValues.PLAIN, CodeChallengeMethodValues.S256].indexOf(
        codeChallengeMethod
      ) < 0
    ) {
...
class Authority {
  ...
  async updateMetadataWithRegionalInformation(metadata) {
    ...
    if (userConfiguredAzureRegion) {
      if (
        userConfiguredAzureRegion !== Constants.AZURE_REGION_AUTO_DISCOVER_FLAG
      ) {
  ...
  updateCloudDiscoveryMetadataFromLocalSources(metadataEntity) {
    ...
    this.logger.verbosePii(
      `Known Authorities: ${
        this.authorityOptions.knownAuthorities || Constants.NOT_APPLICABLE
      }`
    );
    this.logger.verbosePii(
      `Authority Metadata: ${
        this.authorityOptions.authorityMetadata || Constants.NOT_APPLICABLE
      }`
    );
    this.logger.verbosePii(
      `Canonical Authority: ${
  ...
  static replaceWithRegionalInformation(metadata, azureRegion) {
    ...
    regionalMetadata.token_endpoint = Authority.buildRegionalAuthorityString(
          ...
          this.performanceClient?.addFields(
            {
              ...
              requestId: responseHeaders[HeaderNames.X_MS_REQUEST_ID] || '',
              contentTypeHeader:
                responseHeaders[HeaderNames.CONTENT_TYPE] || undefined,
              contentLengthHeader:
                responseHeaders[HeaderNames.CONTENT_LENGTH] || undefined,
...
class PopTokenGenerator {
  ...
  async generateCnf(request, logger) {
    ...
    const reqCnfString = this.cryptoUtils.base64UrlEncode(
...
class ResponseHandler {
  ...
  validateTokenResponse(serverResponse, refreshAccessToken) {
    ...
    if (
      ...
      } - Trace ID: ${serverResponse.trace_id || Constants.NOT_AVAILABLE}`;
  ...
  generateCacheRecord(
    ...
    if (serverTokenResponse.access_token) {
      ...
      const refreshOnSeconds =
        refreshIn && refreshIn > 0 ? reqTimestamp + refreshIn : undefined;
      // non AAD scenarios can have empty realm
  ...
  static async generateAuthenticationResult(
    ...
    if (cacheRecord.accessToken) {
      ...
      extExpiresOn = toDateFromSeconds(
    ...
    if (cacheRecord.appMetadata) {
      familyId =
        cacheRecord.appMetadata.familyId === THE_FAMILY_ID ? THE_FAMILY_ID : '';
    }
    ...
    return {
      ...
      state: requestState
        ...
        : Constants.EMPTY_STRING,
      cloudGraphHostName:
        cacheRecord.account?.cloudGraphHostName || Constants.EMPTY_STRING,
...
class AuthorizationCodeClient extends BaseClient {
  ...
  async createAuthCodeUrlQueryString(request) {
    ...
    const correlationId =
      request.correlationId || this.config.cryptoInterface.createNewGuid();
    this.performanceClient?.addQueueMeasurement(
      PerformanceEvents.AuthClientCreateQueryString,
      correlationId
    );
  ...
  getShrNonce() {
    ...
    if (authenticationInfo) {
      const authenticationInfoChallenges =
        this.parseChallenges(authenticationInfo);
...
function makeExtraSkuString(params) {
  const { skus, libraryName, libraryVersion, extensionName, extensionVersion } =
    params;
...
class ServerTelemetryManager {
  ...
  cacheFailedRequest(error) {
    const lastRequests = this.getLastRequests();
    if (
      lastRequests.errors.length >= SERVER_TELEM_CONSTANTS.MAX_CACHED_ERRORS
    ) {
  ...
  static maxErrorsToSend(serverTelemetryEntity) {
    ...
    for (i = 0; i < errorCount; i++) {
      ...
      const apiId =
        serverTelemetryEntity.failedRequests[2 * i] || Constants.EMPTY_STRING;
      const correlationId =
...
class PerformanceClient {
   ...
   */
  startPerformanceMeasurement(
    measureName, // eslint-disable-line @typescript-eslint/no-unused-vars
...
const hashDoesNotContainKnownProperties =
...
const unableToParseTokenRequestCacheError =
...
const authCodeOrNativeAccountIdRequired =
...
const unableToAcquireTokenFromNativePlatform =
...
const uninitializedPublicClientApplication =
...
const stubbedPublicClientApplicationCalled =
...
class FetchClient {
  ...
  async sendGetRequestAsync(url, options) {
    ...
    try {
      ...
      throw createBrowserAuthError(
        window.navigator.onLine ? getRequestFailed : noNetworkConnectivity
      );
    }
    ...
    try {
      ...
      throw createNetworkError(
        createBrowserAuthError(failedToParseResponse),
        responseStatus,
        responseHeaders
      );
    }
  ...
  async sendPostRequestAsync(url, options) {
    ...
    try {
      ...
      throw createBrowserAuthError(
        window.navigator.onLine ? postRequestFailed : noNetworkConnectivity
      );
    }
    responseHeaders = getHeaderDict(response.headers);
    try {
...
function buildConfiguration(
  ...
  if (
    ...
    logger.warning(
      JSON.stringify(createClientConfigurationError(cannotSetOIDCOptions))
    );
  }
  ...
  async open() {
    return new Promise((resolve, reject) => {
      ...
      openDB.addEventListener('error', () =>
        reject(createBrowserAuthError(databaseUnavailable))
      );
    });
  }
  /**
...
class CryptoOps {
  ...
  async getPublicKeyThumbprint(request) {
    const publicKeyThumbMeasurement = this.performanceClient?.startMeasurement(
    ...
    const unextractablePrivateKey = await importJwk(privateKeyJwk, false, [
      'sign',
    ]);
...
class LocalStorage {
  ...
  async importArray(arr, correlationId) {
    ...
    const promiseArr = [];
    arr.forEach((key) => {
      const promise = this.getItemFromEncryptedCache(key, correlationId).then(
        (value) => {
          if (value) {
            this.memoryStorage.setItem(key, value);
...
function extractBrowserRequestState(browserCrypto, state) {
  ...
  try {
    const requestStateObj = ProtocolUtils.parseRequestState(
      browserCrypto,
      state
    );
...
class BrowserCacheManager extends CacheManager {
  ...
  async hydrateCache(result, request) {
    const idTokenEntity = createIdTokenEntity(
...
const DEFAULT_BROWSER_CACHE_MANAGER = (
  clientId,
  logger,
  performanceClient,
  eventHandler
...
class BaseInteractionClient {
  ...
  async getDiscoveredAuthority(params) {
    ...
    const builtAuthority = Authority.generateAuthority(
      userAuthority,
      params.requestAzureCloudOptions || this.config.auth.azureCloudOptions
    );
 ...
 * @param request
 */
async function initializeBaseRequest(
  request,
  config,
  performanceClient,
  logger
...
function isFatalNativeAuthError(error) {
  if (
    error.ext &&
    error.ext.status &&
    (error.ext.status === PERSISTENT_ERROR || error.ext.status === DISABLED)
  ) {
    return true;
  }
  if (
...
class NativeInteractionClient extends BaseInteractionClient {
  ...
  async handleRedirectPromise(performanceClient, correlationId) {
    ...
    this.browserStorage.removeItem(
      this.browserStorage.generateCacheKey(TemporaryCacheKeys.NATIVE_REQUEST)
    );
    const messageBody = {
      method: NativeExtensionMethod.GetToken,
      request: request,
   ...
   * @returns
   */
  async generateAuthenticationResult(
    response,
    request,
    idTokenClaims,
    accountEntity,
  ...
  cacheNativeTokens(
    ...
    const cachedIdToken = createIdTokenEntity(
      homeAccountIdentifier,
      request.authority,
      response.id_token || '',
      request.clientId,
    ...
    const cachedAccessToken = createAccessTokenEntity(
      homeAccountIdentifier,
      request.authority,
      responseAccessToken,
      request.clientId,
  ...
  onWindowMessage(event) {
    ...
    if (request.body.method === NativeExtensionMethod.HandshakeRequest) {
      ...
      handshakeResolver.reject(
        createBrowserAuthError(nativeExtensionNotInstalled)
      );
    }
  }
  /**
...
class InteractionHandler {
  ...
  async handleCodeResponseFromServer(
    ...
    const tokenResponse = await invokeAsync(
      ...
      this.performanceClient,
      request.correlationId
    )(this.authCodeRequest, authCodeResponse);
  ...
  async acquireToken(request) {
    ...
    try {
      ...
      return await interactionHandler.initiateAuthRequest(navigateUrl, {
        ...
        onRedirectNavigate:
          request.onRedirectNavigate || this.config.auth.onRedirectNavigate,
      });
    } catch (e) {
      if (e instanceof AuthError) {
        e.setCorrelationId(this.correlationId);
    ...
    try {
      ...
      if (
        ...
        if (!loginRequestUrl || loginRequestUrl === 'null') {
          ...
          processHashOnRedirect = await this.navigationClient.navigateInternal(
            homepage,
            navigationOptions
          );
        } else {
 ...
 */
async function initiateAuthRequest(
  requestUrl,
  performanceClient,
  logger,
  correlationId,
 ...
 * @param timeout
 */
async function monitorIframeForHash(
  iframe,
  timeout,
...
class SilentIframeClient extends StandardInteractionClient {
  ...
  async acquireToken(request) {
    ...
    try {
      ...
      const retrySilentRequest = await invokeAsync(
        this.initializeAuthorizationRequest.bind(this),
        PerformanceEvents.StandardInteractionClientInitializeAuthorizationRequest,
        this.logger,
        this.performanceClient,
...
class SilentRefreshClient extends StandardInteractionClient {
  ...
  async acquireToken(request) {
    ...
    return invokeAsync(
      refreshTokenClient.acquireTokenByRefreshToken.bind(refreshTokenClient),
      PerformanceEvents.RefreshTokenClientAcquireTokenByRefreshToken,
      this.logger,
      this.performanceClient,
      request.correlationId
    )(silentRequest).catch((e) => {
      e.setCorrelationId(this.correlationId);
      serverTelemetryManager.cacheFailedRequest(e);
      throw e;
    });
...
class TokenCache {
  ...
  async loadExternalTokens(request, response, options) {
    ...
    const authority = request.authority
      ? new Authority(
          ...
          authorityOptions,
          this.logger,
          request.correlationId || createNewGuid()
        )
      : undefined;
    const cacheRecordAccount = await this.loadAccount(
      request,
  ...
  async loadAccessToken(
    ...
    const extendedExpiresOn =
      ...
      (response.ext_expires_in || response.expires_in) + nowSeconds();
    const accessTokenEntity = createAccessTokenEntity(
      homeAccountId,
      environment,
      response.access_token,
...
class StandardController {
  ...
  async ssoSilent(request) {
    ...
    if (this.canUsePlatformBroker(validRequest)) {
      result = this.acquireTokenNative(validRequest, ApiId.ssoSilent).catch(
        (e) => {
  ...
  async acquireTokenSilentNoIframe(silentRequest, cacheLookupPolicy) {
    ...
    if (
      ...
      return invokeAsync(
        ...
        this.performanceClient,
        silentRequest.correlationId
      )(silentRequest, cacheLookupPolicy).catch((cacheError) => {
        if (cacheLookupPolicy === CacheLookupPolicy.AccessToken) {
          throw cacheError;
        }
        this.eventHandler.emitEvent(
...
function checkIfRefreshTokenErrorCanBeResolvedSilently(
  ...
  const refreshTokenRefreshRequired =
    ...
    refreshTokenError.errorCode === tokenRefreshRequired;
  ...
  const isSilentlyResolvable =
    ...
    refreshTokenError.errorCode === noTokensFound ||
    refreshTokenError.errorCode === refreshTokenExpired;
...
class NestedAppAuthAdapter {
  ...
  fromNaaTokenResponse(request, response, reqTimestamp) {
    ...
    const expiresOn = toDateFromSeconds(
  ...
  fromNaaAccountInfo(fromAccount, idToken, idTokenClaims) {
    ...
    const username =
      fromAccount.username || effectiveIdTokenClaims?.preferred_username || '';
      ...
      switch (error.status) {
        ...
        case BridgeStatusCode.NestedAppAuthUnavailable:
          return new ClientAuthError(
            error.code || nestedAppAuthBridgeDisabled,
            error.description
          );
...
class NestedAppAuthController {
  ...
  async acquireTokenSilentInternal(request) {
    ...
    try {
      ...
      const result = this.nestedAppAuthAdapter.fromNaaTokenResponse(
        naaRequest,
        response,
        reqTimestamp
      );
  ...
  acquireTokenNative(
    request,
    apiId, // eslint-disable-line @typescript-eslint/no-unused-vars
  ...
  acquireTokenByRefreshToken(
    commonRequest, // eslint-disable-line @typescript-eslint/no-unused-vars
  ...
  logoutRedirect(
    logoutRequest // eslint-disable-line @typescript-eslint/no-unused-vars
  ) {
    throw NestedAppAuthError.createUnsupportedError();
  }
  logoutPopup(
...
const stubbedPublicClientApplication = {
  initialize: () => {
    return Promise.reject(
      createBrowserConfigurationAuthError(stubbedPublicClientApplicationCalled)
    );
  },
  acquireTokenPopup: () => {
    return Promise.reject(
...
class BrowserPerformanceClient extends PerformanceClient {
  constructor(configuration, intFields, abbreviations) {
    super(
      ...
      new Logger(configuration.system?.loggerOptions || {}, name, version),
      name,
      version,
    ...
    if (BrowserPerformanceMeasurement.supportsBrowserPerformance()) {
      try {
        measurements.forEach((measurement) => {
          ...
          const entriesForMeasurement = window.performance.getEntriesByName(
            measureName,
            'measure'
          );
